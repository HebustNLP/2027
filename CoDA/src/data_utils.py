...
def infer_struct_type_from_raw_labels(raw_quads):
    def is_null(x):
        return x is None or str(x).lower() == 'null'
    if not raw_quads:
        return 'MIXED'
    all_implicit = all(is_null(q[0]) and is_null(q[3]) for q in raw_quads)
    all_explicit = all((not is_null(q[0])) and (not is_null(q[3])) for q in raw_quads)
    if all_implicit:
        return 'IMPLICIT'
    if all_explicit:
        return 'EXPLICIT'
...
def get_para_targets(sents, labels, data_name, data_type, task, args, n_tuples, wave):
    ...
    return new_sents, targets, order_targets, orders
...
def get_transformed_io(data_path, data_name, data_type, args, n_tuples, wave):
    ...
    if wave == 1:
        new_inputs, targets, order_targets, all_orders = get_para_targets(inputs, labels, data_name,
    ...
    raw_texts = [' '.join(s) for s in inputs]
    return inputs, new_inputs, targets, order_targets, labels, raw_texts, all_orders
...
class ABSADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        item.update({
            "example_id": index,
            "raw_sentence": self.raw_texts[index],
        })

        if self.contrastive_labels is not None:
            item.update({
                "sp_labels": torch.tensor(self.contrastive_labels['sp'][index], dtype=torch.long),
                "ot_labels": torch.tensor(self.contrastive_labels['ot'][index], dtype=torch.long),
                "at_labels": torch.tensor(self.contrastive_labels['at'][index], dtype=torch.long),
            })
        return item

    def _build_examples(self):
        ori_inputs, new_inputs, new_targets, order_targets, raw_labels, raw_texts, all_orders = get_transformed_io(
            self.data_path,
            self.data_name,
            self.data_type,
            self.args,
        ...
        # For the optional supervised contrastive loss.
        # raw_labels is list[list[quad]], where quad is (A, C, S, O) or task-specific order.
        self.raw_texts = raw_texts
        self.all_orders = all_orders
        self.raw_labels = raw_labels
        self.contrastive_labels = None

        if getattr(self.args, "loss_lambda", 0.0) > 0 and self.wave != 1:
            # Label sets: sp = {neg, neu, pos, mixed}, at/ot = {NULL, EXPLICIT, BOTH}
            ...
            ot_labels = []
            at_labels = []
            for ex in raw_labels:
                # ex: list of tuples; each tuple is list/tuple of 4 strings with sentiment in position 2
                sps = set([q[2] for q in ex]) if len(ex) > 0 else set(['neutral'])
                if len(sps) == 1:
                    sp_labels.append(sentiment_dict.get(list(sps)[0], sentiment_dict['neutral']))
                else:
                    sp_labels.append(sentiment_dict['mixed'])

                ots = set([q[3] for q in ex]) if len(ex) > 0 else set(['NULL'])
                if 'NULL' not in ots:
                    ot_labels.append(opinion_dict['EXPLICIT'])
                else:
                    ot_labels.append(opinion_dict['NULL'] if len(ots) == 1 else opinion_dict['BOTH'])

                ats = set([q[0] for q in ex]) if len(ex) > 0 else set(['NULL'])
                if 'NULL' not in ats:
                    at_labels.append(aspect_dict['EXPLICIT'])
                else:
                    at_labels.append(aspect_dict['NULL'] if len(ats) == 1 else aspect_dict['BOTH'])
